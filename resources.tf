...
*Current Value:* {{ if ne .Values.B nil }}{{ printf "%.2f" .Values.B }}{{ else }}No data{{ end }}
  ...
  dynamic "rule" {
    ...
    content {
      ...
      data {
        ...
        model = jsonencode({
          bucketAggs = [
            for agg in rule.value.aggregation : {
              field = agg.field
              id    = agg.id
              settings = {
                min_doc_count = tonumber(agg.min_doc_count)
          ...
          queryType = "lucene"
          timeField = rule.value.time_field
